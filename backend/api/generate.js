...
async function handler(req, res) {
    try {
        // Get style from query parameter
        const style = req.query?.style;
        const validStyles = ["casual", "rare", "formal"];
        if (!validStyles.includes(style)) {
            res.status(400).json({ error: "Invalid or missing style parameter. Use 'casual', 'rare', or 'formal'." });
            return;
        }
        const tableName = style;

        // 1. Get the last generated word from the correct table
        const { rows: lastRows } = await sql.query(
